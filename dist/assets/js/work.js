...
function enableSliderDrag(track) {
  let isDown     = false;
  let isDragging = false;   // true only after pointer has actually moved
  let startX     = 0;
  let startSL    = 0;
  let activeId   = null;    // pointerId stored so we can capture on first move
  ...
  track.addEventListener('pointerdown', (e) => {
    ...
    // setPointerCapture is NOT called here.
    // Capture is set only once a drag is confirmed in pointermove.
  });
  ...
  track.addEventListener('pointermove', (e) => {
    ...
    if (!isDragging) {
      // First real move: commit to drag and capture the pointer.
      isDragging = true;
